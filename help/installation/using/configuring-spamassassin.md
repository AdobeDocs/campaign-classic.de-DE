...
1. Wechseln Sie zu der Datei, in der Sie die Datei entpackt haben, und doppelklicken Sie auf die Datei **run_me.bat** , um das Installationsskript zu starten.
   ...
   Wenn die Windows-Shell nicht angezeigt wird oder nicht angezeigt wird, bevor sie sofort verschwindet, führen Sie die folgenden Schritte aus. Doppelklicken Sie auf die Datei **portableShell.bat** , um eine Windows-Shell anzuzeigen, und überprüfen Sie, ob der Shell-Pfad dem Ordner entspricht, in dem die Datei **spamassassin.zip** entpackt wurde. Ist dies nicht der Fall, greifen Sie mithilfe des Befehls **cd** darauf zu.

   Geben Sie **run_me.bat** ein und klicken Sie dann auf **Enter** , um die Installation und Aktualisierung zu starten. Der Vorgang gibt einen der folgenden Werte zurück, um das Ergebnis der Aktualisierung anzuzeigen.

   * **0**: Es wurde eine Aktualisierung durchgeführt.
   * **1**: Es ist kein neues Update verfügbar.
   * **2**: Es ist kein neues Update verfügbar.
...
Um die Leistung unter Linux zu verbessern, bearbeiten Sie die Datei &quot;**/etc/spamassassin/local.cf**&quot;und fügen Sie am Ende der Datei die folgende Zeile hinzu:
