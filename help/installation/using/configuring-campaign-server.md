...
Im folgenden Abschnitt werden serverseitige Konfigurationen beschrieben, die entsprechend Ihren Anforderungen und Ihren Umgebungsspezifikationen durchgeführt werden können.

Diese Konfigurationen müssen von Administratoren und nur für **lokale** Hostingmodelle durchgeführt werden. Bei **gehosteten** Bereitstellungen können serverseitige Einstellungen nur von Adobe konfiguriert werden. Einige Einstellungen können jedoch in der Systemsteuerung eingerichtet werden (z. B. IP-Whitelist oder URL-Berechtigungen).

Weitere Informationen finden Sie in den folgenden Abschnitten:

* [Dokumentation zur Systemsteuerung](https://docs.adobe.com/content/help/en/control-panel/using/control-panel-home.html)
* [Hosting-Modelle](../../installation/using/hosting-models.md)
* [Campaign Classic-Funktionsmatrix für lokale und gehostete Kampagnen](https://helpx.adobe.com/campaign/kb/acc-on-prem-vs-hosted.html)
* [Konfigurationsschritte für Hybrid- und gehostete Modelle](https://docs.campaign.adobe.com/doc/AC/en/INS_Hybrid_and_Hosted_models_About_hybrid_and_hosted_models.html)

Campaign Classic-Konfigurationsdateien werden im **conf** -Ordner des Adobe Campaign-Installationsordners gespeichert. Die Konfiguration erstreckt sich über zwei Dateien:

* **serverConf.xml**: allgemeine Konfiguration für alle Instanzen. Diese Datei kombiniert die technischen Parameter des Adobe Campaign-Servers: diese werden von allen Instanzen freigegeben. Die Beschreibung einiger dieser Parameter ist nachfolgend beschrieben. Die verschiedenen Knoten und Parameter, die in diesem [Abschnitt](../../installation/using/the-server-configuration-file.md)aufgeführt sind.
...
Operatoren werden über ihr Profil in der Konsole ( **[!UICONTROL Administration > Access management > Operators]** -Knoten) mit einer Sicherheitszone verknüpft. In [diesem Abschnitt erfahren Sie, wie Sie Zonen mit Kampagnenoperatoren verknüpfen](#linking-a-security-zone-to-an-operator).
...
Sicherheitszonen sind miteinander verbunden, was bedeutet, dass die Definition einer neuen Zone innerhalb einer anderen Zone die Anzahl der Operatoren verringert, die sich bei ihr anmelden können, während die jedem Operator zugewiesenen Rechte erhöht werden.
...
Die vordefinierte **Datei &quot;serverConf.xml** &quot;umfasst drei Bereiche: **öffentlich, VPN und LAN**.

>[!NOTE]
>
>**Die vordefinierte Konfiguration ist sicher**. Bevor Sie jedoch von einer früheren Version von Adobe Campaign migrieren, müssen Sie die Sicherheit möglicherweise vorübergehend verringern, um die neuen Regeln migrieren und genehmigen zu können.
...
* Ein Proxy ist in einem Teilnetzwerk einer Sicherheitszone enthalten: Benutzer, die über diesen Proxy Zugriff haben, können unabhängig von ihrem Ursprung auf den Adobe Campaign-Server zugreifen.

   ![](assets/8101_proxy3.png)

Die IP-Adressen der Proxys, die auf den Adobe Campaign-Server zugreifen können, müssen sowohl im betreffenden **`<subnetwork>`** als auch im ersten Teilnetzwerk eingegeben werden **`<subnetwork name="all"/>`**. Beispiel: Hier für einen Proxy mit der IP-Adresse 10.131.146.102:
...
Zuvor müssen Sie die vordefinierte **[!UICONTROL Security zone]** Aufzählung so konfigurieren, dass eine Beschriftung mit dem internen Namen der Zone verknüpft wird, der in der Datei &quot; **serverConf.xml** &quot;definiert ist.

Diese Konfiguration erfolgt im Kampagnen-Explorer:

1. Klicken Sie auf die **[!UICONTROL Administration > Platform > Enumerations]** Node.
1. Wählen Sie die **[!UICONTROL Security zone (securityZone)]** Systemaufzählung aus.

   ![](assets/enum_securityzone.png)

1. Klicken Sie für jede in der Konfigurationsdatei des Servers definierte Sicherheitszone auf die **[!UICONTROL Add]** Schaltfläche.
1. Geben Sie in das **[!UICONTROL Internal name]** Feld den Namen der in der Datei &quot; **serverConf.xml** &quot;definierten Zone ein. Es entspricht dem Attribut **@name** des `<securityzone>` Elements. Geben Sie die mit dem internen Namen verknüpfte Beschriftung in das **** Feld &quot;Beschriftung&quot;ein.

   ![](assets/enum_addsecurityvalue.png)

1. Klicken Sie auf OK und speichern Sie die Änderungen.

Sobald die Zonen definiert und die **[!UICONTROL Security zone]** Aufzählung konfiguriert sind, müssen Sie jeden Operator mit einer Sicherheitszone verknüpfen:

1. Klicken Sie auf die **[!UICONTROL Administration > Access management > Operators]** Node.
1. Wählen Sie den Operator aus, mit dem Sie eine Sicherheitszone verknüpfen möchten, und klicken Sie auf die **[!UICONTROL Edit]** Registerkarte.
1. Gehen Sie zur **[!UICONTROL Access rights]** Registerkarte und klicken Sie auf den **[!UICONTROL Edit access parameters...]** Link.

   ![](assets/zone_operator.png)

1. Wählen Sie eine Zone aus der **[!UICONTROL Authorized connection zone]** Dropdownliste
...
Wenn der 8080-Listening-Anschluss des Tomcat-Servers bereits mit einer anderen Anwendung besetzt ist, die für Ihre Konfiguration erforderlich ist, müssen Sie den 8080-Port durch einen kostenlosen ersetzen (z. B. 8090). Um sie zu ändern, bearbeiten Sie die Datei &quot; **server.xml** &quot;im Ordner **/tomcat-7/conf** des Adobe Campaign-Installationsordners.

Ändern Sie dann den Anschluss der JSP-Relaisseiten. Ändern Sie dazu die Datei &quot; **serverConf.xml** &quot;im Ordner **/conf** des Adobe Campaign-Installationsordners. Alle in der Datei **serverConf.xml** verfügbaren Parameter sind in diesem [Abschnitt](../../installation/using/the-server-configuration-file.md)aufgeführt.
...
Um kundenspezifische Einstellungen zu definieren, können Sie eine Datei **user_contexts.xml** im Ordner **/tomcat-7/conf** erstellen, die auch die Datei **context.xml** enthält.
...
## Anpassen der Bereitstellungsparameter {#personalizing-delivery-parameters}

Die Bereitstellungsparameter werden in der Konfigurationsdatei &quot; **serverConf.xml** &quot;definiert. Alle in der Datei **serverConf.xml** verfügbaren Parameter sind in diesem [Abschnitt](../../installation/using/the-server-configuration-file.md)aufgeführt.

Allgemeine Serverkonfiguration und -befehle werden in der [Campaign-Serverkonfiguration](../../installation/using/campaign-server-configuration.md)ausführlich beschrieben.
...
Es ist jedoch möglich, ihn durch einen Server zu ersetzen, wenn Ihre Sicherheitsrichtlinien dies erfordern. In diesem Fall ist der globale Durchsatz der Durchsatz für das Relais (vorausgesetzt, der Durchsatz für den Server ist niedriger als der für Adobe Campaign).
...
>[!CAUTION]
>
>Dieser Betriebsmodus beinhaltet ernsthafte Beschränkungen der Auslieferung, da er den Durchsatz aufgrund der intrinsischen Performance des Relaisservers (Latenz, Bandwith...) stark reduzieren kann. Darüber hinaus wird die Fähigkeit, synchrone Bereitstellungsfehler zu qualifizieren (durch Analyse des SMTP-Traffics erkannt) begrenzt, und die Versendung ist nicht möglich, wenn der Server nicht verfügbar ist.
...
>[!CAUTION]
>
>Die Affinitätskonfiguration muss von einem Server zum anderen kohärent sein. Es wird empfohlen, sich bei der Affinitätskonfiguration an Adobe zu wenden, da Konfigurationsänderungen auf allen Anwendungsservern, auf denen MTA ausgeführt wird, repliziert werden sollten.
...
1. Um die Affinitätsauswahl in den Dropdown-Listen zu aktivieren, müssen Sie die Affinitätsnamen in die **IPAffinity** -Enumeration einfügen.

   ![](assets/ipaffinity_enum.png)

   >[!NOTE]
   >
   >Aufzählungen sind in [diesem Dokument](../../platform/using/managing-enumerations.md)detailliert.

   Sie können dann die zu verwendende Affinität auswählen, wie unten für Typologien dargestellt:

   ![](assets/ipaffinity_typology.png)

   >[!NOTE]
   >
   >Sie können auch auf die Konfiguration des [Bereitstellungsservers](../../installation/using/email-deliverability.md#delivery-server-configuration)verweisen.
...
* **gehostet**: fügen Sie die URLs hinzu, die über die **Systemsteuerung** zulässig sind. Weitere Informationen finden Sie in der [entsprechenden Dokumentation](https://docs.adobe.com/content/help/en/control-panel/using/instances-settings/url-permissions.html).
...
Die für den mobilen App-Kanal verwendeten JSSPs lauten wie folgt:
...
Es ist möglich, Clientmaschinenverbindungen von außen zu verhindern. Dazu beschränken Sie einfach die Ausführung von **soaprouter.jsp** und genehmigen nur die Ausführung von Spiegelseiten, viralen Links, Web-Formularen und öffentlichen Ressourcen.
...
>[!CAUTION]
>
>Sie sollten kein benutzerdefiniertes Sudo verwenden. Eine Standardsudo muss auf dem System installiert sein.
...
Werden mehrere Server für die Weiterleitung verwendet, müssen sie über SOAP-Aufrufe miteinander kommunizieren können, um Informationen aus den zu umgeleitenden URLs freizugeben. Zum Zeitpunkt des Auslieferungsbeginns ist es möglich, dass nicht alle Umleitungsserver verfügbar sein werden; Sie verfügen daher möglicherweise nicht über den gleichen Informationsstand.
...
Öffentliche Mittel werden in [Verwaltung der öffentlichen Mittel](../../installation/using/deploying-an-instance.md#managing-public-resources)dargestellt.

Sie werden im Installationsordner von Adobe Campaign im Ordner **/var/res/instance** gespeichert.
...
In diesem Fall sollte die neue URL für die öffentlichen Ressourcen, die im oberen Bereich des Fensters des Bereitstellungsassistenten angegeben wird, auf diesen Ordner verweisen.

## Arbeitsabläufe und Affinitäten mit hoher Verfügbarkeit {#high-availability-workflows-and-affinities}

Sie können mehrere Workflow-Server (wfserver) konfigurieren und auf zwei oder mehr Computern verteilen. Wenn Sie diesen Architekturtyp auswählen, konfigurieren Sie den Verbindungsmodus der Lastenausgleicher entsprechend dem Zugriff auf Adobe Campaign.
...
1. Erstellen Sie die Affinitäten des Workflows oder der Aktivität, indem Sie sie in das **[!UICONTROL Affinity]** Feld eingeben.

   Sie können die Affinitätsnamen frei wählen. Achten Sie jedoch darauf, keine Leerzeichen oder Satzzeichen zu verwenden. Wenn Sie verschiedene Server verwenden, geben Sie unterschiedliche Namen an.

   ![](assets/s_ncs_install_server_wf_affinity01.png)

   ![](assets/s_ncs_install_server_wf_affinity02.png)

   Die Dropdownliste enthält zuvor verwendete Affinitäten. Es wird mit der Zeit mit den verschiedenen eingegebenen Werten abgeschlossen.

1. Öffnen Sie die Datei **nl6/conf/config-`<instance>.xml`**.
1. Ändern Sie die Zeile, die dem **[!UICONTROL wfserver]** Modul entspricht, wie folgt:
...
Wenn Sie den Campaign-Server über einen Proxy (z. B. mit einer Dateiübertragungs-Workflow-Aktivität) mit der Außenseite verbinden müssen, müssen Sie den Bereich &quot;proxyConfig&quot;des &quot;serverConf&quot;über einen Befehl konfigurieren. Die folgenden Proxyverbindungen sind möglich: HTTP, HTTPS, FTP, SFTP. Alle in der Datei **serverConf.xml** verfügbaren Parameter sind in diesem [Abschnitt](../../installation/using/the-server-configuration-file.md)aufgeführt.
