---
product: campaign
title: Definieren interaktiver Inhalte in Adobe Campaign Classic
...
Mit Adobe Campaign können Sie das neue interaktive Format [AMP für E-Mail](https://amp.dev/about/email/) nutzen, das unter bestimmten Bedingungen das Senden dynamischer E-Mails ermöglicht.
...
Weitere Informationen zum Testen und Senden von AMP-E-Mails finden Sie unter [diesem Abschnitt](#targeting-amp-email).

Diese Funktion ist über ein dediziertes Package in Adobe Campaign verfügbar. Je nach Ihren Berechtigungen und Ihrem Bereitstellungsmodell können Sie dieses Paket installieren oder sich an Adobe wenden, um es für Sie zu installieren.

>[!NOTE]
>
> Bei hybriden und gehosteten Architekturen muss das Paket auf allen Servern installiert sein, einschließlich der [Mid-Sourcing-Server](../../installation/using/mid-sourcing-server.md) und [Ausführungsinstanz](../../message-center/using/configuring-instances.md#execution-instance).


## Informationen zu AMP for Email {#about-amp-for-email}

Verwenden Sie die **AMP for Email** Neues Format, um AMP-Komponenten in Ihre Nachrichten einzuschließen und das E-Mail-Erlebnis mit komplexen und umsetzbaren Inhalten zu verbessern. Über moderne App-Funktionen, die direkt in E-Mails verfügbar sind, können Empfänger dynamisch mit Inhalten in der Nachricht selbst interagieren.
...
1. Stellen Sie beim Definieren Ihrer Zielgruppe sicher, dass Sie Empfänger auswählen, die das AMP-Format anzeigen können. Siehe [Zielgruppenbestimmung für eine AMP-E-Mail](#targeting-amp-email).
   ...
   >[!NOTE]
   >
   >Derzeit können Sie nur AMP-E-Mails an senden [bestimmte E-Mail-Adressen](#testing-amp-delivery-for-selected-addresses) (für Testzwecke) oder danach [registrierung](#delivering-amp-emails-by-registering) mit den unterstützten E-Mail-Clients.
...
1. Wählen Sie beim Erstellen Ihres E-Mail-Versands eine beliebige Vorlage aus.
   ...
   >[!NOTE]
   >
   >Eine bestimmte AMP-Vorlage enthält ein Beispiel für die Hauptfunktionen, die Sie verwenden können: Produktliste, Karussell, Anmeldung mit zweifacher Bestätigung, Umfrage und erweiterte Serveranforderung.
...
1. Bearbeiten Sie den AMP-Inhalt entsprechend Ihren Anforderungen.
   ...
   Beispielsweise können Sie die Produktlistenkomponente aus der AMP-Vorlage verwenden und eine Liste der Produkte aus einem Drittanbietersystem oder auch innerhalb von Adobe Campaign verwalten. Wenn Sie einen Preis oder ein anderes Element anpassen, wird er automatisch angezeigt, wenn Empfänger die E-Mail über ihr Postfach öffnen.
...
1. Nachdem Sie die Bearbeitung abgeschlossen haben, wählen Sie den gesamten AMP-Inhalt aus und kopieren Sie ihn in den Web-basierten [AMP-Validator](https://validator.ampproject.org) oder eine ähnliche Website.
   ...
   >[!NOTE]
   >
   >Der Adobe Campaign AMP-Editor ist nicht für die Inhaltsvalidierung vorgesehen. Verwenden Sie eine externe Website wie die [Webbasierter AMP-Validator](https://validator.ampproject.org) um zu überprüfen, ob Ihr Inhalt korrekt ist.

1. Nehmen Sie die erforderlichen Änderungen vor, bis der AMP-Inhalt die Validierung besteht.

   ![](assets/amp_validator_pass.png)

1. Um eine Vorschau Ihres Inhalts anzuzeigen, kopieren Sie den validierten Inhalt in [AMP-Playground](https://playground.amp.dev) oder einer ähnlichen Website.
...
Alle Schritte und Spezifikationen, die zum Testen des Versands mit dem AMP-Format auf Gmail-Konten erforderlich sind, werden im entsprechenden Abschnitt beschrieben [Gmail](https://developers.google.com/gmail/ampemail?), [Outlook ](https://docs.microsoft.com/en-gb/outlook/amphtml/)und [Mail.ru](https://postmaster.mail.ru/amp) Entwicklerdokumentation.
