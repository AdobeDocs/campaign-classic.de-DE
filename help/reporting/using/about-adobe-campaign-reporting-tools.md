...
Neben den [nativen Berichten](../../reporting/using/about-campaign-built-in-reports.md) ermöglicht es Adobe Campaign, Berichte in unterschiedlichen Kontexten und den verschiedenen Bedürfnissen entsprechend zu erstellen. In diesem Dokument werden die Grundsätze der Verwendung und die Implementierungsmodi beschrieben.

Adobe Campaign ist keine auf Reporting spezialisierte Anwendung: Die hier erstellten Berichte dienen hauptsächlich der Visualisierung aggregierter Daten. Adobe Campaign-Berichte, die Daten analysieren und darstellen, sind nicht für Datenbankexporte konzipiert.
...
1. **Native Berichte**: Adobe Campaign bietet eine Reihe von Berichten zu Sendungen, Kampagnen, Plattform-Aktivitäten, optionalen Funktionen und mehr. Diese Berichte sind jeweils über die Funktionen verfügbar, auf die sie sich beziehen. Sie können an Ihre individuellen Bedürfnisse angepasst werden.
...
1. **Personalisierte Berichte**: Verwenden Sie Adobe Campaign, um Berichte zu den Daten in der Datenbank zu erstellen. Sobald diese erstellt wurden, sind sie in den entsprechenden Kontexten verfügbar.

   Je nach Komplexität der Abfragen und Berechnungen sowie der Datenmengen werden die in den Berichten zu analysierenden Daten über eine Abfrage gesammelt und in einer Liste (Workflow vom Typ „Daten-Management“) oder einem Cube (mit Marketing Analytics) voraggregiert. Sie wird in Form einer Pivot-Tabelle oder einer Gruppenliste angezeigt.
